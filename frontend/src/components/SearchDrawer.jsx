    ...
    const timer = setTimeout(async () => {
      try {
        ...
        const response = await fetch(
          `${baseUrl}/api/product?search=${encodeURIComponent(searchQuery)}`
        );
      ...
      <div className="px-6 pb-6 mt-3">
        <form onSubmit={handleSearchSubmit} className="relative">
          <input
            ...
            className="w-full px-5 py-2.5 pr-12 border border-gray-400 rounded-full focus:outline-pink-500 text-sm"
          />
        ...
        {products.map((product) => (
          ...
          >
            <img
              ...
              onError={(e) => {
                e.target.src = "https://placeholder.co/64x64?text=No+Image";
              }}
            />

            <div>
              <h4
                className="font-medium text-gray-800 group-hover:text-pink-500 transition-colors"
                style={{ fontFamily: "Playfair Display" }}
              >
                {product.name}
              </h4>
